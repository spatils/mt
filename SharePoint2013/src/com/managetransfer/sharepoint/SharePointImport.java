...
public class SharePointImport {
	...
	public static void main(String[] args) throws Exception {
		...
		shi.setSequenceName(args[0]);
		shi.setSequenceNumber(Integer.parseInt(args[1]));
		shi.setProcessId(Integer.parseInt(args[2]));
		shi.initOperation();
	...
	public void executeOperation() throws Exception {
		...
		for (int i = 0; i < objectList.size(); i++) {
			...
			try {
				...
				sharePointConnection.setBooleanAttributes(rh.getRecord().getListOfBooleanAttributes());
	...
	public void initOperation() throws Exception {
		...
		try {
			try {
				...
				sharePointConnection.baseLibrayPath =destinationPathPh.getParameterValue() ;
				sharePointConnection.setPsw(connectionDetails.getUserPsw());
				sharePointConnection.setUserName(connectionDetails.getUserName());
				sharePointConnection.subSitePath =connectionDetails.getDomain();
				sharePointConnection.setEndPoint(connectionDetails.getUrl());
				sharePointConnection.connectSharePoint2013REST();
				logger.info("Connected Sucessfully to SharePoint");
				rh.setHc(hc);
				...
				ObjectDetails objectDetails = getObjectDetails(targetObjectPh.getParameterValue());
				if(objectDetails==null){
					throw new Exception("Object Details not found");
				}else{
					if ( null != objectDetails.getSharePointUpdateString() && !objectDetails.getSharePointUpdateString().equals("")){
						sharePointConnection.setUpdateMetadataString(objectDetails.getSharePointUpdateString());
					}else{
						throw new Exception("Update String for object not found in object details table ");
					}
				}
				logger.info("Got object details successfuly");
				hc.commitBatchLevelTransaction();
			} catch (Exception e) {
